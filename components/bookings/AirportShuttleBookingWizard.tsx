...
import {
  ...
  TrajetAeroport,
  Ville,
  CreateAirportShuttleBookingDto,
  EmployeeResponse,
  CreateEmployeeDto,
  DepartmentResponse,
  PaymentOption,
  NavetteCICategory,
  NavetteCIQuoteOption,
  NavetteCIQuoteResponse,
  NavetteCIOption,
  NavetteCIPriceResponse,
  CreateNavetteCIBookingDto,
} from "@/lib/api";
...
// Booking response to form data mapper (for edit mode)
const bookingResponseToFormData = (b: BookingResponse): Partial<FormData> => {
  ...
  return {
    ...
    vehiculeId: typeof trajet?.vehiculeId === 'number' ? trajet.vehiculeId : null,
  };
};
...
export default function AirportShuttleBookingWizard({
  ...
  // Detect country
  const isCoteIvoire = selectedPays?.toLowerCase() === "côte d'ivoire" || selectedPays?.toLowerCase() === "cote d'ivoire";
  ...
  const { data: villesResponse } = useQuery({
    ...
    enabled: !!selectedPays && !isCoteIvoire,
  });
  ...
  // Populate selectedDepartId/selectedArriveeId from edit data for Senegal
  useEffect(() => {
  ...
  // ---------- CI Data ----------
  const { data: ciCategoriesRes } = useQuery({
    queryKey: ['ci-categories'],
    queryFn: () => api.bookings.navetteCI.getCategories(),
    enabled: isCoteIvoire,
  });
  const ciCategories: NavetteCICategory[] = ciCategoriesRes?.data ?? [];

  const { data: ciOptionsRes } = useQuery({
    queryKey: ['ci-options'],
    queryFn: () => api.bookings.navetteCI.getOptions(),
    enabled: isCoteIvoire,
  });
  const ciOptions: NavetteCIOption[] = ciOptionsRes?.data ?? [];

  // ---------- Senegal Helpers ----------
  const getVilleName = (v?: Ville | null): string => v?.nom || v?.name || '';

  const villesFromTrajets = useMemo(() => {
  ...
  // ---------- CI Quote & Price ----------
  const ciQuotePayload = useMemo(() => {
    if (!isCoteIvoire || !formData.addressLat || !formData.addressLng || !formData.returnAddressLat || !formData.returnAddressLng || formData.passengers < 1) return null;
    const isFromAirport = formData.direction === 'from_airport';
    const departLat = isFromAirport ? formData.addressLat : formData.returnAddressLat;
    const departLng = isFromAirport ? formData.addressLng : formData.returnAddressLng;
    const arriveeLat = isFromAirport ? formData.returnAddressLat : formData.addressLat;
    const arriveeLng = isFromAirport ? formData.returnAddressLng : formData.addressLng;
    const sens = isFromAirport ? 'airport_to_city' : 'city_to_airport';
    return {
      departLat: departLat!,
      departLng: departLng!,
      ...
      bagages23: formData.bagages23 ?? 0,
      bagages10: formData.bagages10 ?? 0,
      sens,
      isOneWay: !formData.is_round_trip,
      ...(formData.is_round_trip && {
        departRetourLat: arriveeLat!,
        departRetourLng: arriveeLng!,
        arriveeRetourLat: departLat!,
        arriveeRetourLng: departLng!,
      }),
    };
  }, [isCoteIvoire, formData.direction, formData.addressLat, formData.addressLng, formData.returnAddressLat, formData.returnAddressLng, formData.passengers, formData.is_round_trip, formData.bagages23, formData.bagages10]);

  const { data: ciQuoteRes, isFetching: ciQuoteLoading } = useQuery({
    queryKey: ['ci-quote', ciQuotePayload],
    queryFn: () => api.bookings.navetteCI.getQuote(ciQuotePayload!),
    enabled: !!ciQuotePayload,
  });
  const ciQuote: NavetteCIQuoteResponse | undefined = ciQuoteRes?.data;

  const ciPricePayload = useMemo(() => {
    if (!isCoteIvoire || !formData.categoryCode || !ciQuotePayload) return null;
    const options: Array<{ code: string; quantite: number }> = [];
    if (formData.siegeBebes > 0) options.push({ code: 'SIEGE_BEBE', quantite: formData.siegeBebes });
    return {
      ...ciQuotePayload,
      categoryCode: formData.categoryCode,
      options,
      optionsRetour: formData.is_round_trip ? [...options] : undefined,
    };
  }, [isCoteIvoire, formData.categoryCode, ciQuotePayload, formData.siegeBebes, formData.is_round_trip]);

  const { data: ciPriceRes, isFetching: ciPriceLoading } = useQuery({
    queryKey: ['ci-price', ciPricePayload],
    queryFn: () => api.bookings.navetteCI.getPrice(ciPricePayload!),
    enabled: !!ciPricePayload,
  });
  const ciPrice: NavetteCIPriceResponse | undefined = ciPriceRes?.data;

  // ---------- Price Calculation ----------
  const calculateTotal = (): number => {
    if (isCoteIvoire) {
      if (ciPrice) return ciPrice.total;
      const selectedOption = ciQuote?.options?.find(o => o.code === formData.categoryCode);
      return selectedOption?.prix ?? 0;
    } else {
      // Senegal pricing
      if (!selectedTrajet) return 0;
      let total = selectedTrajet.prixAllerSimple ?? selectedTrajet.prix ?? 0;
      if (formData.is_round_trip) {
        total = selectedTrajet.prixAllerRetour || total * 2;
        total = Math.round(total * 0.9); // 10% round-trip discount
      }
  ...
  const handleNext = () => {
    ...
    if (currentStep === 2) {
      if (isCoteIvoire) {
        if (!formData.address || !formData.return_address) { toast.error("Veuillez remplir les deux adresses"); return; }
        if (!formData.addressLat || !formData.returnAddressLat) { toast.error("Adresse invalide, selectionnez une suggestion"); return; }
        if (!formData.departure_date || !formData.departure_time) { toast.error("Date et heure de depart requises"); return; }
        if (formData.direction === 'from_airport' && !formData.flight_number) { toast.error("Numero de vol requis"); return; }
        if (formData.is_round_trip && (!formData.return_date || !formData.return_time)) {
          toast.error("Date et heure de retour requises"); return;
        }
      } else {
        if (!selectedDepartId || !selectedArriveeId) { toast.error("Veuillez selectionner le depart et l'arrivee"); return; }
        if (!formData.departure_date || !formData.departure_time) { toast.error("Date et heure de depart requises"); return; }
        if (!formData.address) { toast.error("Adresse de prise en charge requise"); return; }
        if (formData.direction === 'from_airport' && !formData.flight_number) { toast.error("Numero de vol requis"); return; }
        if (formData.is_round_trip) {
          if (!formData.return_date || !formData.return_time) { toast.error("Date et heure de retour requises"); return; }
          if (!formData.return_address) { toast.error("Adresse de retour requise"); return; }
        }
    ...
    if (currentStep === 3) {
      if (isCoteIvoire) {
        if (!formData.categoryCode) { toast.error("Veuillez choisir une categorie de vehicule"); return; }
      } else {
  ...
  const handleSubmit = () => {
    if (isCoteIvoire) {
      if (!formData.categoryCode || !ciQuotePayload) {
      ...
      const bookingData: CreateNavetteCIBookingDto = {
        categoryCode: formData.categoryCode,
        departLat,
        departLng,
        arriveeLat,
        arriveeLng,
        departAddress: isFromAirport ? formData.address : formData.return_address,
        arriveeAddress: isFromAirport ? formData.return_address : formData.address,
        pax: formData.passengers,
        bagages23: formData.bagages23 ?? 0,
        bagages10: formData.bagages10 ?? 0,
        sens: isFromAirport ? 'airport_to_city' : 'city_to_airport',
        isOneWay: !formData.is_round_trip,
        scheduledDate: formData.departure_date,
        ...
        ...(formData.is_round_trip && {
          scheduledDateRetour: formData.return_date,
          scheduledTimeRetour: formData.return_time,
          departLatRetour: arriveeLat,
          departLngRetour: arriveeLng,
          departAddressRetour: isFromAirport ? formData.return_address : formData.address,
          arriveeLatRetour: departLat,
          arriveeLngRetour: departLng,
          arriveeAddressRetour: isFromAirport ? formData.address : formData.return_address,
        }),
        optionsSelectionnees: formData.siegeBebes > 0 ? [{ optionId: 2, quantite: formData.siegeBebes }] : undefined,
      };

      console.log('[AIRPORT-SHUTTLE CI] Booking data:', JSON.stringify(bookingData, null, 2));
      if (isEdit && bookingId) {
        toast.error("La modification des réservations CI n'est pas encore supportée.");
        return;
      }
      createBooking.mutate(bookingData);
    } else {
      // Senegal booking submission
      ...
      console.log('[AIRPORT-SHUTTLE SN] Booking data:', JSON.stringify(bookingData, null, 2));
      if (isEdit && bookingId) {
  ...
  const createBooking = useMutation({
    mutationFn: (data: any) => {
      if (isCoteIvoire) {
        return api.bookings.navetteCI.create(data);
      } else {
        return api.bookings.createAirportShuttle(data);
      }
    },
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ['bookings'] });
      setBookingRef(response.data?.reference || response.data?.bookingCode || `SUB-${Date.now()}`);
  ...
  const canContinue = (): boolean => {
    switch (currentStep) {
      ...
      case 2:
        if (isCoteIvoire) {
          const baseValid = !!(formData.address && formData.return_address && formData.addressLat && formData.returnAddressLat &&
          ...
          if (formData.is_round_trip) return baseValid && flightValid && !!formData.return_date && !!formData.return_time;
      ...
      case 3:
        if (isCoteIvoire) return !!formData.categoryCode;
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="space-y-1">
                ...
                <p className="font-bold text-base text-slate-900">
                  {isCoteIvoire ? formData.address : getVilleName(selectedTrajet?.villeDepart)}
                </p>
              </div>
              ...
              <div className="space-y-1 text-right">
                ...
                <p className="font-bold text-base text-slate-900">
                  {isCoteIvoire ? formData.return_address : getVilleName(selectedTrajet?.villeArrivee)}
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 bg-white rounded-[2rem] shadow-xl shadow-black/5 p-6 md:p-10">
          <AnimatePresence mode="wait">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <PhoneInput
                      ...
                      error={!!formData.clientPhone && !isValidPhone(formData.clientPhone)}
                    />
                    {formData.clientPhone && !isValidPhone(formData.clientPhone) && (
            ...
            {/* Step 2 - Trajet Senegal */}
            {currentStep === 2 && !isCoteIvoire && (
              <motion.div
                key="step2-sn"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-lg font-bold text-[#171c1f]" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
                      Sens du trajet
                    </h4>
                  </div>
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-lg font-bold text-[#171c1f]">Sens du trajet</h4>
                  </div>
                  <div className="grid grid-cols-2 gap-3 bg-[#f0f4f8] p-2 rounded-2xl">
                    {[
                      { value: "to_airport", label: "Vers l'aeroport", Icon: PlaneTakeoff },
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-center gap-2 p-3 rounded-xl font-semibold ${
                            active ? 'bg-[#ffdbd0] text-[#E04A1F]' : 'text-slate-500'
                          }`}
                        >
                ...
                <div className="space-y-2">
                  <Label className="text-base font-semibold">
                    {formData.direction === 'from_airport' ? "Adresse de prise en charge (aeroport)" : "Adresse de depart (ville)"}
                  </Label>
                  <AddressAutocomplete
                    placeholder={formData.direction === 'from_airport' ? "Aeroport Felix Houphouet-Boigny" : "Ex: Cocody, Abidjan"}
                    value={formData.address}
                    onChange={(v) => handleChange('address', v)}
                    onSelect={(addr, lat, lng) => setFormData(prev => ({ ...prev, address: addr, addressLat: lat, addressLng: lng }))}
                    countryCode={countryNameToCode(selectedPays)}
                    iconColor="text-orange-500"
                  />
                </div>
                ...
                <div className="space-y-2">
                  <Label className="text-base font-semibold">
                    {formData.direction === 'from_airport' ? "Adresse de destination (ville)" : "Adresse d'arrivee (aeroport)"}
                  </Label>
                  <AddressAutocomplete
                    placeholder={formData.direction === 'from_airport' ? "Ex: Plateau, Abidjan" : "Aeroport Felix Houphouet-Boigny"}
                    value={formData.return_address}
                    onChange={(v) => handleChange('return_address', v)}
                    onSelect={(addr, lat, lng) => setFormData(prev => ({ ...prev, return_address: addr, returnAddressLat: lat, returnAddressLng: lng }))}
                    countryCode={countryNameToCode(selectedPays)}
                    iconColor="text-blue-500"
                  />
                </div>
                ...
                <div className="border border-slate-200 rounded-2xl p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button variant="ghost" className="w-full justify-start bg-slate-50 hover:bg-slate-100 rounded-xl h-12 px-4 font-normal">
                            <CalendarIcon className="w-4 h-4 mr-2 text-slate-400" />
                            {formData.departure_date
                              ? format(new Date(formData.departure_date + 'T00:00:00'), "dd/MM/yyyy", { locale: fr })
                              : <span className="text-slate-500">Selectionner une date</span>}
                  ...
                  <div className="border border-blue-200 rounded-2xl p-5 space-y-4 bg-blue-50/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button variant="ghost" className="w-full justify-start bg-white hover:bg-blue-50 rounded-xl h-12 px-4 font-normal">
                              <CalendarIcon className="w-4 h-4 mr-2 text-slate-400" />
                              {formData.return_date
                                ? format(new Date(formData.return_date + 'T00:00:00'), "dd/MM/yyyy", { locale: fr })
                                : <span className="text-slate-500">Selectionner une date</span>}
                ...
                {/* Passengers & Flight & Baggage */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-2">
                    <Label>Passagers</Label>
                    <Select value={formData.passengers.toString()} onValueChange={(v) => handleChange('passengers', parseInt(v))}>
                      <SelectTrigger><Users className="w-4 h-4 mr-2" /><SelectValue /></SelectTrigger>
                      <SelectContent>
                  ...
                  {formData.direction === 'from_airport' && (
                    <div className="space-y-2">
                      <Label>Numero de vol</Label>
                      <Input placeholder="Ex: SN204" value={formData.flight_number} onChange={(e) => handleChange('flight_number', e.target.value)} />
                    </div>
                  )}
                  <div className="space-y-2">
                    <Label>Bagages 23 kg</Label>
                    <Input type="number" min={0} value={formData.bagages23 ?? 0} onChange={(e) => handleChange('bagages23', parseInt(e.target.value) || 0)} />
                  </div>
                  <div className="space-y-2">
                    <Label>Bagages 10 kg</Label>
                    <Input type="number" min={0} value={formData.bagages10 ?? 0} onChange={(e) => handleChange('bagages10', parseInt(e.target.value) || 0)} />
                  </div>
                </div>
              </motion.div>
            )}

            {/* Step 3 - Vehicle Senegal */}
            {currentStep === 3 && !isCoteIvoire && (
              <motion.div
                key="step3-sn"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
            ...
            {currentStep === 3 && isCoteIvoire && (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-[#585e6c] mt-1 font-medium">
                    Selectionnez la categorie de vehicule adaptee a votre trajet.
                  </p>
                </div>

                {ciQuoteLoading && (
                  <div className="text-center py-16">
                    <div className="animate-spin w-8 h-8 border-4 border-[#E04A1F] border-t-transparent rounded-full mx-auto mb-4" />
                    <p className="font-semibold text-slate-700">Calcul du devis en cours...</p>
                  </div>
                )}

                {!ciQuoteLoading && ciQuote && ciQuote.options.length === 0 && (
                  <div className="text-center py-16 bg-slate-50 rounded-3xl">
                    <Car className="w-14 h-14 mx-auto mb-3 text-slate-300" />
                    <p className="font-semibold text-slate-700">Aucune categorie disponible</p>
                    <p className="text-sm mt-1 text-slate-500">Aucune categorie de vehicule n'est disponible pour ce trajet avec ces parametres.</p>
                  </div>
                )}

                {ciQuote && ciQuote.options.length > 0 && (
                  <div className="grid grid-cols-1 gap-4">
                    {ciQuote.options.map((opt) => (
                      <div
                        key={opt.code}
                        onClick={() => handleChange('categoryCode', opt.code)}
                        className={`p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                          formData.categoryCode === opt.code
                      ...
                      >
                        <div className="flex items-center gap-4">
                          {opt.image && (
                            <img src={opt.image} alt={opt.label} className="w-16 h-16 object-cover rounded-xl" />
                          )}
                          <div className="flex-1">
                            <h4 className="font-bold text-lg">{opt.label}</h4>
                            <div className="flex gap-4 text-sm text-slate-600 mt-1">
                              <span>{opt.maxPax} pass. max</span>
                              <span>{opt.maxBagages23kg} bag. 23kg</span>
                              <span>{opt.maxBagages10kg} bag. 10kg</span>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-xl font-extrabold text-[#E04A1F]">{opt.prix.toLocaleString()} FCFA</span>
                            <p className="text-xs text-slate-500">total estime</p>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            {/* Step 4 - Paiement Senegal */}
            {currentStep === 4 && !isCoteIvoire && (
              <motion.div
                key="step4-sn"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                    ...
                    {paymentMethods.map((option) => {
                      ...
                      return (
                        <button
                          type="button"
                          key={option.id}
                          onClick={() => handleChange("payment_method", option.id as PaymentChoice)}
                        ...
                        >
                          ...
                          <p className="font-bold text-lg text-[#171c1f]" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
                            {option.label}
                          </p>
                  ...
                  <div className="space-y-3">
                    ...
                    {formData.is_round_trip && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-emerald-700 font-semibold">-{Math.round(calculateTotal() * 0.1).toLocaleString()} FCFA</span>
                      </div>
                    )}
            ...
            {currentStep === 4 && isCoteIvoire && (
              ...
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {ciOptions.filter(o => o.code === 'SIEGE_BEBE' || o.code === 'ADRESSE_SUPP').map(opt => (
                      <div key={opt.id} className="flex items-center justify-between p-4 rounded-xl border border-slate-200">
                        <div className="flex items-center gap-3">
                          {opt.code === 'SIEGE_BEBE' && <Baby className="w-5 h-5 text-slate-500" />}
                          {opt.code === 'ADRESSE_SUPP' && <Plus className="w-5 h-5 text-slate-500" />}
                          <div>
                            <p className="font-medium text-slate-800">{opt.label}</p>
                            <p className="text-sm text-slate-500">{opt.prix.toLocaleString()} FCFA/unité</p>
                          </div>
                        </div>
                        {opt.code === 'SIEGE_BEBE' && (
                          <Select
                          ...
                          >
                            ...
                            <SelectContent>
                              {Array.from({ length: opt.maxQuantite + 1 }, (_, i) => (
                                <SelectItem key={i} value={i.toString()}>{i}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        )}
                        {opt.code === 'ADRESSE_SUPP' && (
                          <Select
                            value={formData.adresseSupplement.toString()}
                            onValueChange={(v) => handleChange('adresseSupplement', parseInt(v))}
                          >
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between mb-4">
                            <div
                              className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                                selected ? "bg-[#ffdbd0] text-[#E04A1F]" : "bg-[#dfe3e7] text-slate-500"
                              }`}
                            >
                              {isCompany ? <Users className="w-6 h-6" /> : <User className="w-6 h-6" />}
                            </div>
                ...
                {/* CI Price Summary */}
                <div className="bg-slate-50 rounded-2xl p-6">
                  <h3 className="font-semibold text-slate-800 mb-4">Resume du prix</h3>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-600">Distance aller</span>
                      <span className="font-medium text-slate-800">{ciQuote?.distanceKm ?? '—'} km</span>
                    </div>
                    {ciPrice ? (
                      <>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-600">Prix categorie</span>
                          <span className="font-medium text-slate-800">{ciPrice.prixAller.toLocaleString()} FCFA</span>
                        </div>
                        {ciPrice.aller.montantOptions > 0 && (
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-slate-600">Options aller</span>
                            <span className="text-slate-800">+{ciPrice.aller.montantOptions.toLocaleString()} FCFA</span>
                          </div>
                        )}
                        {ciPrice.retour && ciPrice.retour.montantOptions > 0 && (
                          <div className="flex items-center justify-between text-sm">
                            <span className="text-slate-600">Options retour</span>
                            <span className="text-slate-800">+{ciPrice.retour.montantOptions.toLocaleString()} FCFA</span>
                          </div>
                        )}
                        <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                          <span className="text-lg font-semibold text-slate-800">Total</span>
                          <span className="text-2xl font-bold text-[#E04A1F]">{ciPrice.total.toLocaleString()} FCFA</span>
                        </div>
                      </>
                    ) : (
                      <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                        <span className="text-lg font-semibold text-slate-800">Total estime</span>
                        <span className="text-2xl font-bold text-[#E04A1F]">{calculateTotal().toLocaleString()} FCFA</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="p-6 rounded-xl bg-slate-50 space-y-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-[#ffdbd0] text-orange-700 font-medium">
                        {formData.is_round_trip ? 'Aller-retour' : 'Aller simple'}
                      </span>
                    </div>
                    <p className="font-medium text-slate-800">
                      {isCoteIvoire
                        ? `${formData.address} → ${formData.return_address}`
                ...
                <div className="p-6 rounded-xl bg-[#ffdbd0]/40 border-2 border-orange-200">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-orange-300">
                    ...
                    <span className="text-3xl font-bold text-[#E04A1F]">{calculateTotal().toLocaleString()} FCFA</span>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          {currentStep === 5 ? null : currentStep === 1 ? (
            <>
              ...
              <div className="bg-[#ffdbd0] rounded-[2rem] p-6 relative overflow-hidden">
                ...
                <h4 className="text-xl font-extrabold text-[#3a0a00] mt-4 mb-2" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
              ...
              <div className="rounded-[1.5rem] overflow-hidden relative h-44 group bg-gradient-to-br from-[#171c1f] to-[#2c3134]">
                ...
                <div className="absolute inset-0 flex items-end p-5">
                  <div>
                    <p className="text-white font-bold text-lg leading-tight" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
            ...
            <div className="bg-white rounded-[2rem] shadow-2xl shadow-black/5 p-6">
              <h3 className="text-xl font-bold text-[#171c1f] mb-6" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
                Resume du trajet
              </h3>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="p-4 bg-[#f0f4f8] rounded-2xl space-y-3 text-sm">
                  ...
                  {!isCoteIvoire && selectedTrajet?.vehicule && (
                    <div className="flex justify-between gap-2">
                  ...
                  {isCoteIvoire && formData.categoryCode && (
                    <div className="flex justify-between gap-2">
                      <span className="text-slate-500 shrink-0">Categorie</span>
                      <span className="font-bold text-[#171c1f] text-right truncate">
                        {ciQuote?.options?.find(o => o.code === formData.categoryCode)?.label || formData.categoryCode}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-3 border-t border-[#dfe3e7] pt-6">
                  ...
                  <>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium text-[#171c1f]">
                        {selectedTrajet
                          ? `${(formData.is_round_trip
                              ? (selectedTrajet.prixAllerRetour || (selectedTrajet.prixAllerSimple ?? selectedTrajet.prix ?? 0) * 2)
                    ...
                    {formData.is_round_trip && (
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-semibold text-emerald-700">
                          -{Math.round(calculateTotal() * 0.1).toLocaleString()} FCFA
                        </span>
                      </div>
                    )}
                  </>
                ...
                {isCoteIvoire && ciPrice && (
                  <>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-slate-500">Prix categorie</span>
                      <span className="text-sm font-medium text-[#171c1f]">{ciPrice.prixAller.toLocaleString()} FCFA</span>
                    </div>
                    {ciPrice.aller.montantOptions > 0 && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-500">Options aller</span>
                        <span className="text-sm font-medium text-[#171c1f]">+{ciPrice.aller.montantOptions.toLocaleString()} FCFA</span>
                      </div>
                    )}
                    {ciPrice.retour && ciPrice.retour.montantOptions > 0 && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-500">Options retour</span>
                        <span className="text-sm font-medium text-[#171c1f]">+{ciPrice.retour.montantOptions.toLocaleString()} FCFA</span>
                      </div>
                    )}
                  </>
                )}
                ...
                <div className="flex justify-between items-end pt-4 border-t border-[#dfe3e7]">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-black text-[#E04A1F] tracking-tight" style={{ fontFamily: "Manrope, system-ui, sans-serif" }}>
